...
def validate(self,method):
    ...
    if self.custom_aetas_coupon_code:
        total = self.grand_total if self.disable_rounded_total == 1 else self.rounded_total
...
def on_submit(self,method):
    ...
    applied = True
    coupon_name = frappe.db.get_value("AETAS Coupon Code", {"coupon_code": self.custom_aetas_coupon_code})
    if not coupon_name:
        applied = False
        # This shouldn't happen because we validated earlier, but guard anyway
        frappe.throw(_("Coupon code not found: {0}").format(self.custom_aetas_coupon_code))
    ...
    if coupon.get("status") == "Used":
        applied = False
        frappe.throw(_("Coupon {0} is already used.").format(self.custom_aetas_coupon_code))
    if applied:
...
def validate_coupon_code(coupon_code, items, grand_total):
    ...
    # Basic checks
    if not coupon_code:
        return {"status": "Invalid", "message": "Coupon code is required.", "total_discount": 0.0, "final_total": grand_total, "breakdown": []}

    try:
        items_list = json.loads(items)
    except Exception:
        return {"status": "Invalid", "message": "Invalid items payload (must be JSON).", "total_discount": 0.0, "final_total": grand_total, "breakdown": []}

    # 1. Verify coupon exists and is active
    coupon = frappe.db.get_value("AETAS Coupon Code", {"coupon_code": coupon_code}, ["name", "status"], as_dict=1)
    if not coupon:
        return {"status": "Invalid", "message": "Invalid Coupon Code", "total_discount": 0.0, "final_total": grand_total, "breakdown": []}
    if coupon.get("status") != "Active":
        return {"status": "Inactive", "message": "Coupon Code is not Active", "total_discount": 0.0, "final_total": grand_total, "breakdown": []}
    ...
    for row in items_list:
        ...
        if item_code is None or base_amount is None:
            ...
            return {"status": "Invalid", "message": "Each item must include item_code and base_amount", "total_discount": 0.0, "final_total": grand_total, "breakdown": []}
        item_rows.append({"item_code": item_code, "base_amount": to_number(base_amount)})
        item_codes.add(item_code)

    if not item_rows:
        return {"status": "Invalid", "message": "No items provided", "total_discount": 0.0, "final_total": grand_total, "breakdown": []}

    # 3. Resolve item_code -> item_group in one DB call
    item_group_map = {}
    item_codes_list = list(item_codes)
    # Use get_all for efficiency
    fetched_items = frappe.get_all("Item", filters=[["name", "in", item_codes_list]], fields=["name", "item_group"])
